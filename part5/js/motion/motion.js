...
Blocker.prototype = {
	...
	block:function (e){
		...
		*/
		if((this.lastY-e.y) < -180)
		// If the lastY - e.y (current Y) is less than -180 (e.g. lastY = 10˚ and e.y = 350˚)
		{
			/*
			*	This means that the user if moving to the RIGHT past 2πr
			*/
...
function Inertia(easeFunction, duration, scalar){
	...
	//	Is the app tracking the delta 变量增量
		...
		var eventCatcher = function (e){
			...
			if(e.type.match(/move/i)){
				...
				if(self.trackDelta){
					...
					console.log("selt.post(onMotionUpdate)");
